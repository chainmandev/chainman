...
Import `scripts/chainman.just` from the project's `justfile`. This checked-in file
is generated by the pinned runtime; project behavior is declared in `chainman.toml`.
It uses host shebang recipes and passes literal arguments to the launcher, so a
project's default Just shell does not add another Nix entry. Ordinary commands do
not rewrite the facade. Runtime updates regenerate it, including declared runtime
copies and templates, in the same verified transaction as the lock and bootstrap.
...
`just deps-update resume=/absolute/transaction/directory`. Resume checks the original
...
Maintainers can regenerate the facade explicitly with the pinned runtime's
`scripts/recipes.py ROOT`, or use `--check` for an exact generated-file contract.
