...
class NixDirectoryTests(unittest.TestCase):
    def setUp(self):
        self.temporary = tempfile.TemporaryDirectory(prefix="chainman nix paths ")
        self.addCleanup(self.temporary.cleanup)
            ...
            json.dumps(
                {
                    ...
                    "nodes": {
                        ...
                        "nixpkgs": {"locked": {"lastModified": 0}},
                    },
                }
            )
        )

    def test_default_root_and_nested_directory_run_in_selected_flake(self):
        for relative in (None, ".", "tooling/nix with spaces"):
            with self.subTest(directory=relative):
                directory = self.root / (relative or "nix")
                self.flake(directory)
                spec = {} if relative is None else {"directory": relative}
                env = {"CHAINMAN_UPDATE_ACTIVE": "1"}
                with (
                    patch.object(
                        updates.registry, "data", return_value=[self.selected]
                    ) as fetch,
                    patch.object(updates, "managed_run") as run,
                ):
                    updates.update_nix(self.root, {"nix": spec}, self.now, env)
                ...
                run.assert_called_once_with(
                    [
                        ...
                        "--override-input",
                        "nixpkgs",
                        "github:NixOS/nixpkgs/" + self.selected["sha"],
                    ],
                    cwd=directory,
                    env=env,
                    check=True,
    ...
    def reject(self, directory):
        with (
            patch.object(updates.registry, "data") as fetch,
            patch.object(updates, "managed_run") as run,
            self.assertRaises(ValueError),
        ):
            updates.update_nix(
                self.root, {"nix": {"directory": directory}}, self.now, {}
            )
        fetch.assert_not_called()
        run.assert_not_called()

    def test_unsafe_or_missing_directory_rejected_before_resolution(self):
        for relative in (
            "",
            7,
            str(self.root),
    ...
    def test_symlink_directory_and_ancestor_rejected(self):
        ...
        self.flake(outside / "nix")
        (self.root / "linked").symlink_to(outside, target_is_directory=True)
        self.reject("linked")
        self.reject("linked/nix")

    def test_symlink_flake_inputs_rejected_even_for_root_flake(self):
        outside = self.root.parent / "outside"
        self.flake(outside)
        for relative in (".", "nix"):
            directory = self.root / relative
            self.flake(directory)
            for filename in ("flake.lock", "flake.nix"):
                with self.subTest(directory=relative, filename=filename):
                    path = directory / filename
                    original = path.read_bytes()
                    path.unlink()
                    path.symlink_to(outside / filename)
                    self.reject(relative)
                    path.unlink()
